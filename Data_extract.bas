...
ThisWorkbook.Connections("Hist").OLEDBConnection.CommandText = _
        "SELECT HISTDATE " & _
        "FROM ismrt.ismrt_tbl_stg_raw_eqplog_ne " & _
        "GROUP BY HISTDATE " & _
        "Union ALL " & _
        "SELECT histdate " & _
        "FROM ismrt.ismrt_tbl_stg_raw_eqplognehist " & _
        "Group BY histdate " & _
...
With ThisWorkbook.Sheets("TABLE2_NONEXPOSURE")
.Cells(WorksheetFunction.CountA(.Columns(1)) + 1, 1).Value = .Cells(WorksheetFunction.CountA(.Columns(1)), 1)
.Cells(WorksheetFunction.CountA(.Columns(1)), 4).Value = "Dummy"
End With

'MsgBox "Done!", , "Project_Refresh"
End Sub
Sub get_data()
